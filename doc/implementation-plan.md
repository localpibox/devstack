...
- Lemonade extension sets `maxTokens = contextWindow × 0.125` (~32k for 262k window)
- Pi sends `max_tokens: 32k` on every request
- Compaction fires at 93.8% (too late) with thinking blocks bloating the prompt
- Combined: prompt + max_tokens exceeds the hard window limit
...
| `.env` | Already 0.125 → change to 0.06 | 1.4 |
| `versions.env` | Add ratio tracking | 1.4 |
| `README.md` | Update ratio examples | 1.3 |
...
- [ ] Change `ENV LPB_MAX_TOKENS_CONTEXT_RATIO` in Dockerfile to 0.06
- [ ] Change default in `support/start.sh` to 0.06
- [ ] Update `.env` from 0.125 to 0.06
- [ ] Update `.env.example` comment
- [ ] Update `versions.env` with ratio tracking
- [ ] Update `README.md` examples
- [ ] Build image, verify maxTokens in model mapping is ~15k

### Session 2: Lemonade Extension
- [ ] Clone/pull `localpibox/lemonade-pi-plugin` fork
- [ ] Change `maxTokens` calculation for Qwen reasoning models (0.06 ratio)
- [ ] Update `syncModelStore()` with same change
- [ ] Clarify `reasoning_effort` comment
- [ ] Test with running container — verify model shows 15k maxTokens

### Session 3: Pi Fork — Compaction
- [ ] Clone/pull `localpibox/pi` fork
- [ ] Disable thinking during compaction (`reasoning: "off"`)
- [ ] Double `reserveTokens` to 32768
- [ ] Test: sessions should stay under context limit longer

### Session 4: Pi Fork — Per-Request Clamping (optional, future)
- [ ] Implement `clampOutputTokensToWindow()` in agent-harness.ts
- [ ] Add `SAFETY_MARGIN = 2048`, `MIN_OUTPUT_TOKENS = 4096`
- [ ] Test with long sessions, verify no overflow errors
...
2. **Sessions stay stable** through 100+ turns without overflow
...
6. **Documentation exists** explaining the rationale and configuration
