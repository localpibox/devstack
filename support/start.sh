...
if [[ -z "$MODE" ]]; then
    echo "[devstack] ERROR: --mode cli|web is required" >&2
    echo "Usage: start.sh run --mode <cli|web> [--verbose] [-- arg1 ...]" >&2
    exit 1
fi
...
info()  { echo "[devstack] $*"; }
debug() { [[ "$VERBOSE" = "true" ]] && echo "[devstack][debug] $*" || true; }
warn()  { echo "[devstack] ⚠ $*" >&2; }
err()   { echo "[devstack] ✗ $*" >&2; }

# ─── 1. DEFAULT VALUES (lowest precedence) ──────────────────────────────────
# These are the fallback values if nothing else sets them.
...
export LPB_STATE_DIR="${LPB_STATE_DIR:-/home/dev/.pi}"
